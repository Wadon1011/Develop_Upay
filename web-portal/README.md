...
研究室メンバーがGoogleアカウントでサインインし、自分の残高（スプレッドシート上の`UserData`シートの値）を確認し、自分のユーザーID（5桁固定）のQRコードを表示・保存できる、シンプルなWebポータルのデモです。表示されたQRコードはUPay端末のカメラでそのまま読み取れます。
...
7. フロントエンドは残高を表示し、ユーザーIDが返ってきていれば`qrcodejs`でQRコードを描画します。QRコードの中身はユーザーIDの文字列そのもので、UPay端末は`int.tryParse()`で数値化して`UserData.id`と直接照合します（暗号化なし）。QRコード画像はPNGとして保存できます。
...
4. 対象スプレッドシートの`UserData`シートに、少なくとも「メールアドレス列」「残高列」「ユーザーID列」を用意する（列名の例: `Email` / `メールアドレス`、`Balance` / `残高`、`UserID`。詳細候補は`Code.gs`内の`EMAIL_HEADER_CANDIDATES` / `BALANCE_HEADER_CANDIDATES` / `USERID_HEADER_CANDIDATES`を参照）。QRコードに載せる桁数は`Code.gs`内の`USERID_DIGITS`（既定`5`）で調整できます。
5. 右上の「デプロイ」→「新しいデプロイ」。
6. 種類の選択で「ウェブアプリ」を選択。
