...
研究室メンバーがGoogleアカウントでサインインし、自分の残高（スプレッドシート上の`UserData`シートの値）を確認できるだけの、シンプルなWebポータルのデモです。
...
web-portal/
├── frontend/
│   └── index.html   # Googleサインイン + 残高表示のフロントエンド
├── backend/
│   └── Code.gs       # Google Apps Script（IDトークン検証 + スプレッドシート検索）
└── README.md
```
...
6. 該当行があれば残高を、なければエラーをJSONで返し、フロントエンドに表示します。
...
4. 対象スプレッドシートの`UserData`シートに、少なくとも「メールアドレス列」と「残高列」を用意する（列名の例: `Email` / `メールアドレス`、`Balance` / `残高`。詳細候補は`Code.gs`内の`EMAIL_HEADER_CANDIDATES` / `BALANCE_HEADER_CANDIDATES`を参照）。
5. 右上の「デプロイ」→「新しいデプロイ」。
6. 種類の選択で「ウェブアプリ」を選択。
