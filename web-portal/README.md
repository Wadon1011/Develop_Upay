...
web-portal/
├── frontend/
│   └── index.html   # Googleサインイン + 残高表示 + ワンタイムQRコード生成
├── backend/
│   ├── Code.gs       # ポータルAPIとFlutterレジの購入・チャージAPI
│   └── appsscript.json # SheetsサービスとOAuthスコープ
└── README.md
```

## このデモの制限事項

- Web画面では残高・履歴の確認とQR表示を行います。購入・チャージ操作はFlutterレジから行います。
- GASバックエンドはFlutterレジと共有します。サービスアカウントの秘密鍵には依存しません。
- レジAPIはスプレッドシートの残高・在庫・履歴を書き換えます。GASの実行アカウントには編集権限が必要です。
...
- 検証済みGoogleアカウントのメールアドレスを `UserData` の行に照合し、その行の `UserID` とQRトークンを紐付けます。
- FlutterレジはQR検証・購入・チャージ・ギフト受領をGAS経由で行い、このポータルと同じスプレッドシートを参照します。設定は [レジ接続手順](../docs/gas-register.md) を参照してください。

- 補充希望リアクション・在庫予約は、初回利用時に `ItemReactions`・`Reservations` シートをGASが自動作成します。
- 予約の作成・取消と補充リアクションの更新は `LockService` で排他制御します。
- 商品一覧の `stock` は有効な予約を差し引いた数で、Flutterの商品一覧にも反映されます。ただし購入APIの在庫判定は `ItemData.Stock` を使用し、予約の消化や予約者優先の購入判定はまだ行いません。
...
9. Flutterレジが `verifyQrToken` を呼び、本人の情報とレジ用セッションを取得します。購入時には `purchase` APIが残高・在庫・履歴を更新します。レジAPIキーはGASのスクリプトプロパティ `REGISTER_API_KEY` に設定します。
...
2. `Code.gs` を `backend/Code.gs` の内容で置き換え、`backend/appsscript.json` のSheets v4サービスとOAuthスコープも反映します。詳しくは [レジ接続手順](../docs/gas-register.md) を参照してください。
...
6. 種類の選択で「ウェブアプリ」を選択。
   ...
   - アクセスできるユーザー: 全員（ログインなしで到達可能）。ポータルAPIはIDトークン、レジAPIはレジキーとQRセッションで認証します。
7. デプロイすると **ウェブアプリのURL**（`https://script.google.com/macros/s/xxxxxxxxxxxx/exec`の形式）が発行されるので控える。
8. 実行アカウントに対象スプレッドシートの編集権限を付与します。残高更新には閲覧権限だけでは足りません。
